...
class PostDiscussionFragment : BasePostFragment() {
    ...
    override fun postContent() {
        ...
        if (userDetails != null) {
            ...
            Log.d("PostDiscussionFragment", "Discussion: $discussion")
            val discussionRepository = DiscussionRepository()
            val currentList = sharedViewModel.retrievedDiscussionList.value?.toMutableList() ?: mutableListOf()
            discussionRepository.postDiscussion(discussion) { postedDiscussion ->
                if (postedDiscussion != null) {
                    currentList.add(postedDiscussion)
                    sharedViewModel.retrievedDiscussionList.value = currentList
                    Toast.makeText(context, "Discussion posted successfully", Toast.LENGTH_SHORT).show()
                } else {
                    Toast.makeText(context, "Failed to post discussion", Toast.LENGTH_SHORT).show()
                }
            }
        } else {
